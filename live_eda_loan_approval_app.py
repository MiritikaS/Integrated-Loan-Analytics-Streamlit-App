...
DATA_PATH = BASE_DIR / "data" / "loan_risk_cleaned.csv"
MODEL_PATH = BASE_DIR / "model_outputs" / "loan_approval_model.pkl"
...
def load_data():
    if not DATA_PATH.exists():
        st.error("Data file not found. Please upload data/loan_risk_cleaned.csv")
        st.stop()
...
if not DATA_PATH.exists():
    st.error("Data file was not found. Please upload data/loan_risk_cleaned.csv")
    st.stop()
...
with tab2:
    ...
    with c1:
        st.subheader("Loan Product Distribution")
    ...
    with c2:
        st.subheader("Secured vs Unsecured Trade Lines")

        secured_df = approval_order(
            filtered_df.groupby("Approved_Flag", observed=False)[["Secured_TL", "Unsecured_TL"]]
            .sum()
            .reset_index()
        )

        secured_long = secured_df.melt(
            id_vars="Approved_Flag",
            value_vars=["Secured_TL", "Unsecured_TL"],
            var_name="Exposure Type",
            value_name="Trade Lines"
        )
    ...
    with c1:
        st.subheader("Average Enquiries by Approval Tier")

        enq_df = approval_order(
            filtered_df.groupby("Approved_Flag", observed=False)["tot_enq"]
            .mean()
            .reset_index()
        )
    ...
    with c2:
        st.subheader("Average Delinquency by Approval Tier")

        delinq_df = approval_order(
            filtered_df.groupby("Approved_Flag", observed=False)["num_times_delinquent"]
            .mean()
            .reset_index()
        )
